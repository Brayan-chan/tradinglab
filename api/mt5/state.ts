...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!accountResult.data) return res.status(200).json({ ok:true, account:null, positions:[], deals:[], symbols:[] })
  const key = accountResult.data.account_key
  const [positions, deals, symbols] = await Promise.all([
  ...
  if (positions.error || deals.error || symbols.error) return res.status(500).json({ ok:false, error:'State lookup failed' })
  return res.status(200).json({ ok:true, account:accountResult.data, positions:positions.data, deals:deals.data, symbols:symbols.data })
}
