...
export function validSnapshot(value: unknown): value is Mt5Snapshot {
  ...
  return v.positions.every(p => p && text(p.ticket,32) && text(p.symbol,40) && (p.side==='buy'||p.side==='sell') && [p.volume,p.priceOpen,p.priceCurrent,p.sl,p.tp,p.profit,p.swap].every(finite))
    && v.deals.every(d => d && text(d.ticket,32) && text(d.orderTicket,32) && text(d.positionTicket,32) && text(d.symbol,40) && (d.side==='buy'||d.side==='sell') && (d.entry==='in'||d.entry==='out'||d.entry==='inout') && text(d.time,40) && [d.volume,d.price,d.profit,d.commission,d.swap].every(finite))
