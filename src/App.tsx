...
import { Activity, AlertTriangle, BarChart3, BookOpen, Calculator, Check, ChevronRight, CircleDollarSign, FileUp, FlaskConical, LayoutDashboard, LoaderCircle, LockKeyhole, Menu, Play, Radio, Shield, ShieldAlert, Square, Target, TrendingDown, TrendingUp, X } from 'lucide-react'
...
function MonteCarlo() {
  ...
  const [capital,setCapital]=useState(1000),[risk,setRisk]=useState(1),[win,setWin]=useState(38),[ratio,setRatio]=useState(2),[costR,setCostR]=useState(0)
  const [frequency,setFrequency]=useState<Frequency>('week'),[perPeriod,setPerPeriod]=useState(3),[months,setMonths]=useState(36),[ruin,setRuin]=useState(50),[simulations,setSimulations]=useState(1000)
  const [result,setResult]=useState<MonteCarloResult|null>(null),[running,setRunning]=useState(false),[progress,setProgress]=useState(0),[error,setError]=useState('')
  const workerRef=useRef<Worker|null>(null),jobRef=useRef(0)
  const rawOperations=Math.max(1,Math.round(perPeriod*months*(frequency==='day'?21:frequency==='week'?52/12:1))),operations=Math.min(5000,rawOperations)
  const periodLabel=frequency==='day'?'día':frequency==='week'?'semana':'mes',durationLabel=months<12?`${months} meses`:`${(months/12).toFixed(months%12?1:0)} años`
  useEffect(()=>()=>workerRef.current?.terminate(),[])
  function run(){workerRef.current?.terminate();const worker=new Worker(new URL('./workers/monteCarlo.worker.ts',import.meta.url),{type:'module'}),id=++jobRef.current;workerRef.current=worker;setRunning(true);setProgress(0);setError('')
    const input:MonteCarloInput={capital,riskPercent:risk,winRate:win,rewardRiskRatio:ratio,operations,simulations,ruinDrawdown:ruin,costR}
    worker.onmessage=(event:MessageEvent<{id:number;type:string;progress?:number;result?:MonteCarloResult;error?:string}>)=>{if(event.data.id!==id)return;if(event.data.type==='progress')setProgress(event.data.progress??0);if(event.data.type==='result'&&event.data.result){setResult(event.data.result);setProgress(100);setRunning(false);worker.terminate()}if(event.data.type==='error'){setError(event.data.error??'No se pudo simular.');setRunning(false);worker.terminate()}}
    worker.onerror=()=>{setError('El proceso de simulación se detuvo inesperadamente.');setRunning(false);worker.terminate()};worker.postMessage({id,input,seed:20260908+id})}
  function cancel(){workerRef.current?.terminate();workerRef.current=null;setRunning(false);setError('Simulación cancelada.')}
  return <section className="page"><div className="page-title"><div><span className="eyebrow">LABORATORIO DE INCERTIDUMBRE</span><h1>La misma ventaja. Miles de futuros distintos.</h1><p>Separa cuentas simuladas de operaciones y mide si la ventaja sobrevive a las rachas.</p></div>{running?<button className="secondary mc-cancel" onClick={cancel}><Square size={16}/> Cancelar</button>:<button className="primary" onClick={run}><Play size={18}/> Simular</button>}</div>
    <div className="mc-grid"><article className="panel mc-controls"><h2>Parámetros</h2><NumberField label="Capital inicial" value={capital} onChange={setCapital} suffix="USD"/><NumberField label="Riesgo por operación" value={risk} onChange={setRisk} suffix="%" step="0.05"/><NumberField label="Tasa de acierto" value={win} onChange={setWin} suffix="%"/><NumberField label="Ratio riesgo:beneficio" value={ratio} onChange={setRatio} suffix=":1" step="0.1"/><NumberField label="Costes por operación" value={costR} onChange={setCostR} suffix="R" step="0.01"/>
      <label className="field"><span>Cuentas simuladas</span><div><select value={simulations} onChange={e=>setSimulations(Number(e.target.value))}><option value={1000}>1,000 · rápida</option><option value={10000}>10,000 · confiable</option><option value={100000}>100,000 · estrés</option></select></div></label>
      <div className="frequency-control"><span>¿Cada cuánto operas?</span><div className="segmented triple">{(['day','week','month'] as Frequency[]).map(value=><button key={value} className={frequency===value?'active':''} onClick={()=>setFrequency(value)}>{value==='day'?'Día':value==='week'?'Semana':'Mes'}</button>)}</div></div>
      <NumberField label={`Operaciones por ${periodLabel}`} value={perPeriod} onChange={setPerPeriod} step="1"/><NumberField label="Duración" value={months} onChange={setMonths} suffix="meses" step="1"/><NumberField label="Cuenta reventada al caer" value={ruin} onChange={setRuin} suffix="% DD"/>
      <div className={`simulation-summary ${rawOperations>5000?'warning':''}`}><b>≈ {durationLabel}</b><span>{rawOperations.toLocaleString('es-MX')} operaciones calculadas</span>{rawOperations>5000&&<small>Se usarán 5,000: límite seguro del modelo.</small>}</div>
      <button className="primary wide" onClick={run} disabled={running}>{running?<><LoaderCircle className="spin" size={17}/> Simulando {progress.toFixed(0)}%</>:<><Activity size={17}/> Ejecutar simulación</>}</button></article>
      <div className="mc-output">{error&&<div className="notice error"><AlertTriangle size={17}/>{error}</div>}{running&&<div className="progress-track"><i style={{width:`${progress}%`}}/></div>}{!result&&!running?<article className="panel mc-empty"><FlaskConical/><h2>Configura y ejecuta el experimento</h2><p>El cálculo se realizará fuera de la interfaz para que la pestaña permanezca estable.</p></article>:result&&<><div className="metric-grid mc"><MetricCard label="Esperanza neta" value={`${result.expectancyR>=0?'+':''}${result.expectancyR.toFixed(3)}R`} detail={`Break-even ${pct(result.breakEvenWinRate)}`} tone={result.expectancyR>0?'good':'bad'} icon={CircleDollarSign}/><MetricCard label="Cuentas reventadas" value={pct(result.ruinProbability)} detail={`de ${result.simulations.toLocaleString('es-MX')} cuentas · DD ${ruin}%`} tone={result.ruinProbability>5?'bad':'good'} icon={ShieldAlert}/><MetricCard label="Mediana final" value={money.format(result.medianEnding)} detail={`P5 ${money.format(result.p05)} · P95 ${money.format(result.p95)}`} tone={result.medianEnding>capital?'good':'bad'} icon={BarChart3}/><MetricCard label="Rentables" value={pct(result.profitableProbability)} detail={`${result.operations.toLocaleString('es-MX')} operaciones por cuenta`} tone={result.profitableProbability>50?'good':'warn'} icon={Target}/></div>
      <div className="mc-detail-grid"><article className="panel"><span>Mejor cuenta</span><strong className="good-text">{money.format(result.bestEnding)}</strong></article><article className="panel"><span>Peor cuenta</span><strong className="bad-text">{money.format(result.worstEnding)}</strong></article><article className="panel"><span>Drawdown P95</span><strong>{pct(result.p95Drawdown)}</strong></article><article className="panel"><span>Racha de pérdidas P95</span><strong>{Math.round(result.p95LossStreak)} operaciones</strong></article></div>
      <article className="panel chart-panel"><div className="panel-head"><div><span className="eyebrow">DISTRIBUCIÓN</span><h2>Percentiles de equity</h2></div><span className="legend multi"><i></i>P5 <i></i>Mediana <i></i>P95</span></div><ResponsiveContainer width="100%" height={330}><LineChart data={result.paths}><CartesianGrid stroke="#222832" vertical={false}/><XAxis dataKey="operation" stroke="#6f7784"/><YAxis stroke="#6f7784" tickFormatter={v=>`$${Math.round(v)}`}/><Tooltip contentStyle={{background:'#11151b',border:'1px solid #29303b',borderRadius:12}} formatter={v=>money.format(Number(v))}/><Line dataKey="conservative" dot={false} stroke="#ff5b58"/><Line dataKey="median" dot={false} stroke="#ff7b3d" strokeWidth={2}/><Line dataKey="optimistic" dot={false} stroke="#38d99b"/></LineChart></ResponsiveContainer></article>
      <article className="panel dd-panel"><div><span className="eyebrow">PRUEBA DE ESTRÉS</span><h2>Probabilidad de alcanzar cada drawdown</h2></div><div className="dd-grid">{result.drawdownProbabilities.map(item=><div key={item.level}><span>DD ≥ {item.level}%</span><strong className={item.probability>20?'bad-text':item.probability>5?'orange':'good-text'}>{pct(item.probability)}</strong></div>)}</div><p>Drawdown mediano {pct(result.medianDrawdown)} · Ruina media {result.averageOperationsToRuin?`${Math.round(result.averageOperationsToRuin)} operaciones`:'no observada'}.</p></article></>}</div></div>
  </section>
}
