...
input long AllowedDemoLogin = 0; // 0 disables demo orders even when other switches are enabled
...
input double MaxSpreadDistancePercent = 20.0; // spread máximo permitido como % de la distancia entrada->stop
...
void DailyStats(double &net,int &consecutiveLosses){
  ...
  datetime fromServer=TimeCurrent()-(TimeGMT()-fromGmt);historyHealthy=HistorySelect(fromServer,TimeCurrent());if(!historyHealthy)return;
...
double VolumeForRisk(double entry,double stop,bool buy){
  double step=SymbolInfoDouble(StrategySymbol,SYMBOL_VOLUME_STEP),minimum=SymbolInfoDouble(StrategySymbol,SYMBOL_VOLUME_MIN),maximum=SymbolInfoDouble(StrategySymbol,SYMBOL_VOLUME_MAX),lossPerLot=0;
  if(step<=0||minimum<=0||maximum<minimum||entry==stop||!OrderCalcProfit(buy?ORDER_TYPE_BUY:ORDER_TYPE_SELL,StrategySymbol,1.0,entry,stop,lossPerLot)||lossPerLot>=0)return 0;
  double budget=AccountInfoDouble(ACCOUNT_EQUITY)*RiskPercent/100.0,volume=MathFloor((budget/(-lossPerLot))/step+1e-9)*step;
  volume=MathMin(volume,maximum);
  if(volume<minimum)return 0;
  double confirmedLoss=0;
  if(!OrderCalcProfit(buy?ORDER_TYPE_BUY:ORDER_TYPE_SELL,StrategySymbol,volume,entry,stop,confirmedLoss)||confirmedLoss>=0||-confirmedLoss>budget+0.000001)return 0;
  return volume;
}
...
void Evaluate(){
  ...
  double distance=MathAbs(entry-stop);if(distance<=0){SendDecision("blocked",buy?"buy":"sell","Stop técnico inválido",closedBar,entry,stop,EMPTY_VALUE,spreadPoints,h1Fast,h1Slow,m15Ema,m15Atr,m5Ema1);return;}
  // El spread se compara contra la distancia de stop de ESTE trade, no contra el ATR: un spread fijo del bróker
  // no mide iliquidez, pero sí puede distorsionar el riesgo real si el stop queda demasiado cerca del precio.
  if((tick.ask-tick.bid)>distance*MaxSpreadDistancePercent/100.0){string detail=StringFormat("Spread %.2f > tope %.2f (distancia stop %.2f; límite %.1f%% de la distancia); no se evaluó señal",tick.ask-tick.bid,distance*MaxSpreadDistancePercent/100.0,distance,MaxSpreadDistancePercent);SendDecision("blocked",buy?"buy":"sell",detail,closedBar,entry,stop,EMPTY_VALUE,spreadPoints,h1Fast,h1Slow,m15Ema,m15Atr,m5Ema1);return;}
  double target=buy?entry+distance*RewardRisk:entry-distance*RewardRisk;
